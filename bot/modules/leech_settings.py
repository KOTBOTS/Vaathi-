...
def leechSet(update, context):
    ...
    msg = f"Leech Type for {user_id} is "
    if (
...
def setThumb(update, context):
    ...
    if reply_to is not None and reply_to.photo:
        ...
        os.remove(photo_dir)
        sendMessage(f"Custom thumbnail saved for <code>{user_id}</code>.", context.bot, update)
    else:
        sendMessage("Reply to a photo to save as custom thumbnail!", context.bot, update)
